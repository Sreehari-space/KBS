...
totals like `13.469999999999999`, and the receipt then *back-computes* the subtotal with
...
`Math.round(qty * unitPricePaise)` so the *money* stays integral even though the quantity isn't.
...
> mode is on, tax is *extracted* from the price, not added on top. Getting this backwards
